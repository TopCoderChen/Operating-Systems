...
int th_cd(char **argv)
{
    ...
    setenv("PWD", getcwd( buffer, MAX_PATH_LENGTH), 1); //chdir will not change pwd, we need change it ourselves
...
void redirection(process* p) // parse command that might contains < > .
{
    char *position, *tp;
    char *r = "<>>";
    char *rn= "012";
    ...
    strcpy(p->infile, file[0]);
//    printf("infile :%s |%d\n", p->infile, (int)(strlen(p->infile )));
    strcpy(p->outfile,file[1]);
//    printf("outfile:%s |%d\n", p->outfile,(int)(strlen(p->outfile)));
    strcpy(p->errfile,file[2]);
...
void pipeline(job* j, char *cmd)
{
    ...
    cmd_start = cmd;
    lastp = j->head_process;// lastp always points to the last process in the job LinkedList
