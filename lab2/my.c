...
void *malloc(size_t size) {
    ...
    while (bkeep != NULL) {
        if (bkeep->free_count) { //free_count is not 0
            ...
            if( bkeep->free_count == 128/(1<<offset)-1 ){ //this is a whole-superblock
...
void free(void *ptr) {
    ...
    while (levels[bkeep->level].whole_superblocks > RESERVE_SUPERBLOCK_THRESHOLD) {
        ...
        int diff=levels[bkeep->level].whole_superblocks - RESERVE_SUPERBLOCK_THRESHOLD;
        
        struct superblock_bookkeeping *node1 = levels[bkeep->level].next;
        struct superblock_bookkeeping *node2 = levels[bkeep->level].next;
        
        while(diff>=1){
            
            node2 = node1;
            
            if(node2->free_count== 128/(1<<bkeep->level)-1 ){
                //remove current node2, and
                //update freelist and bkeep LinkedList
                //then munmap
                struct superblock_bookkeeping * nextSb = node2->next;
                node1->next = nextSb;
                munmap( node2, 1<< (bkeep->level+5) );
                levels[bkeep->level].whole_superblocks--;
                levels[bkeep->level].free_objects -= max;
                diff--;
            }
